...
class KnowledgeGraphMigration:
    ...
    def _analyze_current_schema(self) -> dict:
        ...
        stats["needs_migration"] = not (
            stats["has_temporal"] and stats["has_causal"] and stats["has_strength"]
        )
    ...
    def _execute_schema_migration(self, dry_run: bool):
        ...
        if dry_run:
            ...
            logger.info(
                f"  SQL statements: {len([s for s in migration_sql.split(';') if s.strip()])}"
            )
            return
    ...
    def _validate_migration(self):
        ...
        if relation_count > 0 and strength_count == relation_count:
            ...
            logger.warning(
                f"  ⚠ {relation_count - strength_count} relations missing strength"
            )
    ...
    def _generate_report(self):
        ...
        report["statistics"]["entities_by_type"] = {
        ...
        report["statistics"]["avg_relationship_strength"] = (
            round(avg_strength, 3) if avg_strength else 0
        )
        ...
        logger.info("\nMigration Summary:")
        logger.info(
            f"  Total entities: {sum(report['statistics']['entities_by_type'].values())}"
        )
        ...
        logger.info(
            f"  Bidirectional relations: {report['statistics']['bidirectional_relations']}"
        )
        logger.info(
            f"  Avg relationship strength: {report['statistics']['avg_relationship_strength']}"
        )
