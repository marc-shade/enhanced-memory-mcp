...
async def migrate_batch(
    nmf, entities: List[Dict[str, Any]], batch_num: int, total_batches: int
):
    ...
    logger.info(
        f"Processing batch {batch_num}/{total_batches} ({len(entities)} entities)..."
    )
    ...
    for entity in entities:
        try:
            ...
            result = await nmf.remember(
                content=content, metadata=metadata, agent_id="migration"
            )
    ...
    logger.info(
        f"Batch {batch_num} complete: {successful}/{len(entities)} successful, {failed} failed"
    )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Migrate memory-db entities to Qdrant (direct access)"
    )
    parser.add_argument(
        "--batch-size", type=int, default=20, help="Batch size for migration"
    )
    parser.add_argument(
        "--limit", type=int, default=None, help="Limit number of entities to migrate"
    )
    parser.add_argument(
        "--db-path",
        type=str,
    ...
    try:
        ...
        if nmf.embedding_manager:
            ...
            print(
                f"✅ {len(providers)} embedding providers available: {', '.join(providers)}"
            )
        else:
        ...
        for i in range(0, len(entities), batch_size):
            ...
            successful, failed, providers = await migrate_batch(
                nmf, batch, batch_num, total_batches
            )
            ...
            if batch_num % 10 == 0:
                ...
                print()
                print(
                    f"Progress: {total_successful + total_failed}/{len(entities)} processed"
                )
        ...
        print(
            f"Successful: {total_successful} ({total_successful / len(entities) * 100:.1f}%)"
        )
