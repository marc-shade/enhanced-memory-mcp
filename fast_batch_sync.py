...
async def process_entity(
    session: aiohttp.ClientSession, entity: Dict[str, Any]
) -> bool:
...
async def process_batch(
    session: aiohttp.ClientSession, entities: List[Dict[str, Any]]
) -> tuple:
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(
            f"{QDRANT_BASE_URL}/collections/enhanced_memory"
        ) as resp:
            data = await resp.json()
        ...
        for i in range(0, len(entities), BATCH_SIZE):
            ...
            print(
                f"   Batch {batch_num}/{total_batches}: {success}/{len(batch)} succeeded | "
    ...
    print(
        f"Qdrant points: {initial_count} → {final_count} (+{final_count - initial_count})"
    )
