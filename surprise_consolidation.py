...
CONSOLIDATION_CONFIG = {
    ...
    # Override with SURPRISE_METRICS_FILE; defaults beside the memory database.
    "metrics_file": os.environ.get(
        "SURPRISE_METRICS_FILE",
        str(
            Path(
                os.environ.get("ENHANCED_MEMORY_DIR")
                or os.environ.get("MEMORY_DIR")
                or (Path.home() / ".claude" / "enhanced_memories")
            )
            / "surprise_consolidation_metrics.jsonl"
        ),
    ),
}
...
class SurpriseConsolidator:
    ...
    def _create_scorer(self) -> SurpriseBasedMemory:
        ...
        return SurpriseBasedMemory(embedding_fn=embedding_fn, search_fn=search_fn)

    def consolidate_episodic_memories(
        self, episodic_memories: List[Dict], time_window_hours: int = 24
    ) -> Dict:
        """
        ...
        logger.info(
            f"Starting surprise-based consolidation of {len(episodic_memories)} memories"
        )
        ...
        recent_memories = [
            ...
            if datetime.fromisoformat(m.get("created_at", datetime.now().isoformat()))
            > cutoff
        ]

        logger.info(
            f"Filtered to {len(recent_memories)} memories in {time_window_hours}h window"
        )
        ...
        for memory in recent_memories:
            ...
            if (
                score.should_store
                or score.score
                >= CONSOLIDATION_CONFIG["high_surprise_immediate_promote"]
            ):
                promoted.append(
                    {
                        "memory": memory,
        ...
        if len(promoted) > 0:
            ...
            if (
                current_semantic_count + len(promoted)
                > CONSOLIDATION_CONFIG["max_semantic_memories"]
            ):
                ...
                logger.info(
                    f"Retention gate triggered: forgetting {len(forgotten)} memories"
                )
        ...
        logger.info(
            f"Consolidation complete: promoted={len(promoted)}, skipped={len(skipped)}, "
    ...
    def _apply_retention_gate(self, count_to_remove: int) -> List[str]:
        ...
        try:
            ...
            results = self.qdrant_client.scroll(
                collection_name="semantic_memories", limit=10000, with_payload=True
            )
            ...
            if candidates:
                self.qdrant_client.delete(
                    collection_name="semantic_memories", points_selector=candidates
                )
    ...
    def get_consolidation_stats(self, days: int = 7) -> Dict:
        ...
        try:
            ...
            avg_surprise = sum(
                m["average_surprise_score"] for m in recent_metrics
            ) / len(recent_metrics)
            ...
            return {
                ...
                "promotion_rate": total_promoted / total_evaluated
                if total_evaluated > 0
                else 0,
                "average_surprise_score": avg_surprise,
                "retention_gates_triggered": sum(
                    1 for m in recent_metrics if m["retention_gate_triggered"]
                ),
            }
...
if __name__ == "__main__":
    ...
    for p in result["promoted"]:
        print(
            f"  - {p['memory']['content'][:50]}... (score={p['surprise_score'].score:.2f})"
        )
