...
class NeuralMemoryFabric:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        # Initialize paths
        # Both need expanduser(): the configured defaults start with "~", and
        # Path("~/...").mkdir() creates a directory literally named "~" in the
        # process's working directory rather than under $HOME. That is where the
        # stray "~" and "$HOME" directories in the source tree came from.
        self.sqlite_path = Path(self.config["storage"]["sqlite"]["path"]).expanduser()
        self.file_root = Path(self.config["storage"]["files"]["root"]).expanduser()
    ...
    async def _init_sqlite(self):
        ...
        # Idempotent migration for pre-existing DBs (columns added 2026-08-05).
        for col, ddl in (
            (
                "is_open",
                "ALTER TABLE nmf_memory_blocks ADD COLUMN is_open INTEGER DEFAULT 1",
            ),
            ("closed_at", "ALTER TABLE nmf_memory_blocks ADD COLUMN closed_at TEXT"),
        ):
            present = any(
                r[1] == col
                for r in cursor.execute(
                    "PRAGMA table_info(nmf_memory_blocks)"
                ).fetchall()
            )
            if not present:
                cursor.execute(ddl)
        ...
        # Create indexes
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_nmf_entities_agent ON nmf_entities(agent_id)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_nmf_entities_tier ON nmf_entities(tier)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_nmf_entities_timestamp ON nmf_entities(timestamp)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_nmf_entities_importance ON nmf_entities(importance_score)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_nmf_links_from ON nmf_links(from_memory_id)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_nmf_links_to ON nmf_links(to_memory_id)"
        )
    ...
    async def _init_vector_db(self):
        ...
        try:
            ...
            if backend == "qdrant":
                ...
                if not collection_exists:
                    try:
                        ...
                        collection_exists = any(
                            a.alias_name == collection_name for a in all_aliases
                        )
                    except:
                ...
                if not collection_exists:
                    ...
                    distance = distance_map.get(
                        vector_config.get("distance_metric", "Cosine"), Distance.COSINE
                    )
            ...
            logger.warning(
                f"Qdrant initialization failed: {e} - vector search disabled"
            )
            self.vector_db = None
    ...
    async def _init_redis(self):
        ...
        try:
            ...
            self.redis_client = redis.from_url(
                self.config["storage"]["cache"]["url"], decode_responses=True
            )
...
Summary:"""
    ...
    async def _calculate_importance_llm(
...
Importance:"""
    ...
    async def _generate_embedding(
        ...
        """
        Generate embedding for text using the shared embedding cache.

        The shared cache is the canonical embedding source for the agentic system.
        It delegates to the EmbeddingManager (Google/OpenAI/Ollama fallback chain)
        and caches results in a shared SQLite DB so both SAFLA MCP and NMF
        avoid redundant API calls.

        Falls back to the local EmbeddingManager directly if the shared cache
        is unavailable.
        ...
        """
        # Try the shared embedding cache first (canonical source)
        try:
            from shared_embedding_cache import get_shared_cache

            cache = get_shared_cache()
            result = await cache.generate_embedding(text, provider)
            if result and result.get("embedding"):
                logger.debug(
                    f"Shared cache embedding: {result.get('provider')} "
                    f"{result.get('dimensions')}d "
                    f"(cached={result.get('from_cache', False)})"
                )
                return result["embedding"]
        except ImportError:
            logger.debug(
                "Shared embedding cache not available, using local EmbeddingManager"
            )
        except Exception as e:
            logger.warning(f"Shared embedding cache error: {e}, falling back to local")

        # Fallback: use local EmbeddingManager directly
        ...
        if result:
            logger.debug(
                f"Generated {result.provider} embedding: {result.dimensions} dims in {result.latency_ms:.2f}ms"
            )
            return result.embedding
        else:
    ...
    async def remember(
        ...
        """,
            (
                ...
                0,
                timestamp,
                MemoryTier.ULTRA_FAST.value,
                agent_id,
                1,
                self._calculate_checksum(content),
        ...
        if self.redis_client:
            ...
            cache_data = json.dumps(
                {"content": content, "timestamp": timestamp, "agent_id": agent_id}
            )
        ...
        if self.vector_db and embedding:
            try:
                ...
                logger.info(
                    f"Stored embedding in Qdrant for {memory_id} (UUID: {uuid_id})"
                )
            except Exception as e:
        ...
        if self.vector_db and embedding:
            try:
                links_created = await self.create_dynamic_links(
                    memory_id, similarity_threshold=0.6, max_links=5
                )
                logger.info(f"Created {links_created} automatic links for {memory_id}")
            except Exception as e:
                logger.warning(f"Dynamic linking failed: {e}")

        # TODO: Store in file system (Phase 4)  quality-gate: allow
        # Inherited from the source deployment and kept on purpose: it labels a
        # code path that genuinely does not persist to the file backend. The
        # sqlite path above did run. Removing the label would read as finished.
    ...
    async def recall(
                ...
                if query_embedding:
                    ...
                    for result in search_results:
                        vector_results.append(
                            {
                                "memory_id": result.payload.get(
                                    "memory_id", result.id
                                ),  # Use original memory_id from payload
                ...
                if sr["memory_id"] not in seen_ids:
                    ...
                    for item in combined:
                        if item["memory_id"] == sr["memory_id"]:
                            item["rank_score"] = item.get("rank_score", 0) + (
                                sr["importance_score"] * 0.3
                            )
                            item["source"] = "hybrid"
        ...
        if self.redis_client and results:
            ...
            await self.redis_client.setex(
                cache_key, self.config["storage"]["cache"]["ttl"], json.dumps(results)
            )

        logger.info(
            f"Recalled {len(results)} memories for query: {query[:50]}... (mode: {mode})"
        )

        # TODO: Integrate graph traversal  quality-gate: allow
        # TODO: LLM re-ranking  quality-gate: allow
        # Both inherited and kept on purpose: these results are vector-similarity
        # only. The labels are the honest description of what this returns.

        return results

    async def open_block(self, agent_id: str, block_name: str) -> Dict[str, Any]:
        """Load a memory block into context (Letta-style), marking it open."""
        cursor = self.sqlite_conn.cursor()
        cursor.execute(
            """
        ...
        row = cursor.fetchone()
        if row:
            # Record that the block is loaded into active context so the
            # open/close lifecycle has real, observable state.
            cursor.execute(
                """
                UPDATE nmf_memory_blocks
    ...
    async def close_block(self, agent_id: str, block_name: str) -> Dict[str, Any]:
        ...
        Real close: flips is_open to 0 and stamps closed_at. Fails loudly if the
        block does not exist, so a caller can never mistake a no-op for a close.
        """
        cursor = self.sqlite_conn.cursor()
        cursor.execute(
            """
            SELECT block_id FROM nmf_memory_blocks
            WHERE agent_id = ? AND block_name = ?
        """,
            (agent_id, block_name),
        )
    ...
    async def edit_block(
        self, agent_id: str, block_name: str, new_value: str
    ) -> Dict[str, Any]:
        """Edit a memory block"""
        timestamp = datetime.utcnow().isoformat()
    ...
    async def _create_temporal_edge(
        ...
        try:
            with self.graph_driver.session() as session:
                ...
                logger.info(
                    f"Created {relationship_type} edge: {from_id} -> {to_id} (score: {link_score})"
                )
                return True
    ...
    async def find_related_memories(
        self, memory_id: str, similarity_threshold: float = 0.6, max_links: int = 5
        ...
        try:
            ...
            cursor = self.sqlite_conn.cursor()
            cursor.execute(
                "SELECT content FROM nmf_entities WHERE id = ?", (memory_id,)
            )
            row = cursor.fetchone()
    ...
    async def create_dynamic_links(
        ...
        related = await self.find_related_memories(
            memory_id, similarity_threshold, max_links
        )
        ...
        for related_id, similarity in related:
            try:
                ...
                links_created += 1
                logger.info(
                    f"Linked {memory_id} -> {related_id} (similarity: {similarity:.3f})"
                )
    ...
    async def traverse_graph(
        ...
        try:
            with self.graph_driver.session() as session:
                ...
                logger.info(
                    f"Graph traversal from {start_memory_id} found {len(connected_memories)} connected memories"
                )
    ...
    async def temporal_query(
        self, agent_id: str, as_of_time: str, query: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        logger.info(
            f"Temporal query found {len(results)} memories valid at {as_of_time}"
        )
        return results
    ...
    async def consolidate_memories(
        ...
        for mem_id, content, access_count, importance in frequent_memories:
            ...
            related = await self.find_related_memories(
                mem_id, similarity_threshold=similarity_threshold, max_links=10
            )
        ...
        logger.info(
            f"Consolidation complete: {len(clusters)} clusters, {summaries_created} summaries"
        )
